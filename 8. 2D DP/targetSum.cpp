...
public:
    int findTargetSumWays(vector<int> &nums, int target)
    {
        ...
        // if target is greater than range (target sum)  <--- edge cases
        if (target > sum)
            return 0;

        // if range can't be divided   <--- edge cases
        if ((target + sum) % 2 != 0)
            return 0;
        ...
        // if required range is negative  <--- edge cases
        if (s < 0)
            return 0;
...
int main()
{
    ...
    Solution s;
    cout << s.findTargetSumWays(nums, k) << endl;
